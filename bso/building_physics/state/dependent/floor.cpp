#ifndef BSO_BP_FLOOR_CPP
#define BSO_BP_FLOOR_CPP

namespace bso { namespace building_physics { namespace state { namespace dependent {

floor::floor(const unsigned int& index, bso::utilities::geometry::polygon* geometry,
	...
	mConstruction(construction), mSide1(side1), mSide2(side2)
{
	...
	else
	{
		...
		errorMessage << "\nError, trying to assign transition resistance to side 1\n"
								 << "of a floor but could not recognize the type of the adjacent state\n"
								 << "(bso/building_physics/state/dependent/floor.cpp)" << std::endl;
...
bso::utilities::geometry::polygon* floor::getGeometry() const
{
	try
	{
		mGeometry->end();
	...
	{
		std::stringstream errorMessage;
		errorMessage << "\nError, requesting geometry pointer from floor, but it does not\n"
