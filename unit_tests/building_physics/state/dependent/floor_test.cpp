...
#define BOOST_TEST_MODULE "bp_dep_state_floor_test"
#endif
...
BOOST_AUTO_TEST_SUITE( bp_dep_state_floor_test )
	...
	BOOST_AUTO_TEST_CASE( initialize )
	{
		...
		BOOST_REQUIRE_NO_THROW(dependent::floor f1(1,q1,con1,&wp1,&gp1));
		dependent::floor f1(12,q1,con1,&wp1,&gp1);
		
		BOOST_REQUIRE(f1.getIndex() == 12);
		BOOST_REQUIRE( f1.isDependent());
		BOOST_REQUIRE(!f1.isIndependent());
		BOOST_REQUIRE(!f1.isSpace());
		BOOST_REQUIRE( f1.isFloor());
		BOOST_REQUIRE(!f1.isWall());
		BOOST_REQUIRE(!f1.isWindow());
		BOOST_REQUIRE(!f1.isWeatherProfile());
		BOOST_REQUIRE(!f1.isGroundProfile() );
		BOOST_REQUIRE(abs(f1.getCapacitance()/(413.1e-3)-1) < 1e-9);
		BOOST_REQUIRE(abs(f1.getThickness()/(300)-1) < 1e-9);
		BOOST_REQUIRE(abs(f1.getArea()/(1e-6)-1) < 1e-9);
		BOOST_REQUIRE(f1.getGeometry()->isSameAs(*q1));
		BOOST_REQUIRE(f1.getVisualizationProperties().first == "brown");
	}
	...
	BOOST_AUTO_TEST_CASE( init_and_update_system )
	{
		...
		dependent::floor f1(3,q1,con1,&wp1,&gp1);
